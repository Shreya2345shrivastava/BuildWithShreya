	...
	return (
		<label className="flex w-full flex-col gap-2">
			...
			<textarea
				disabled={disabled}
				className={cn(
					"min-h-[var(--form-textarea-min-height)] w-full rounded-[var(--radius-md)] border bg-[var(--color-surface-elevated)] px-4 py-3 text-form text-[var(--color-text-primary)] placeholder:text-[var(--color-text-muted)] transition-soft focus:border-[var(--color-focus)] focus:outline-none focus:ring-2 focus:ring-[color-mix(in_srgb,var(--color-focus)_28%,transparent)] disabled:cursor-not-allowed disabled:bg-[var(--color-border-subtle)] disabled:text-[var(--color-text-muted)]",
